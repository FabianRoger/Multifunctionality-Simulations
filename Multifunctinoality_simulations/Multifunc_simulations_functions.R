...
AverageFunction <- function(SPM, FUNC, method = "average", selfunc = "F 1", selfac = 1,
  ...
  #   r: growth rate of amplification factor (how fast it growth to CF)
  ...
  # define function to be applied to each plot for method "sum"
  if (is.na(pmatch(method, c("sum"))) == FALSE) {
    MeanFunc <- function(x) {
      colSums(FUNC[FUNC$Species %in% spec[which(x == 1)], ][,-1])
    } 
  }
  
  # define function to be applied for method "density" 
  if (is.na(pmatch(method, c("density"))) == FALSE) {
    MeanFunc <- function(x) {
      Com <- FUNC[FUNC$Species %in% spec[which(x == 1)], ][,-1]
