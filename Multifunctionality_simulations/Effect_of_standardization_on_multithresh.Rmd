...
+ `Single_dist`: 
All function values are drawn from a single distribution. **Choosing the pre-set values produces Figure S2**

+ `Multi_dist`:
Different parameters can be specified for each function (which are supplied to the same distribution).

+ `Biodepth_data`:
The monoculture data from the BIODEPTH experiment (included in the `multifunc` package) are used to simulate the null-expectation for the multithreshold approach. **Choosing Germany produces Figure 3**
...
library(repmis) # to download Biodepth data  
...
Single_dist <- TRUE #produces Figure S2
Multi_dist <- FALSE
Biodepth_data <- FALSE #produces Figure 3
Biodepth_sim <- FALSE
...
# download biodepth data from github repository of multifunc package
  
source_data("https://github.com/jebyrnes/multifunc/blob/master/data/all_biodepth.rda?raw=True")
...
maxrep <-  max(sapply(1:specnum, function(x) choose(specnum,x)))
#maxrep <-  500
...
if(Biodepth_data) {
  ggsave("Figure_3.pdf", plot, height = 6, width = 10)
}

if(Single_dist) {
  ggsave("Figure_S2.pdf", plot, height = 6, width = 10)
}
